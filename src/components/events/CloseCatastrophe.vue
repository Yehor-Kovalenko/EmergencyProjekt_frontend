    ...
    setup() {
      ...
      const translations = {
        pl: {
          ...
          closeError: 'Wystąpił błąd podczas zamykania katastrofy. Spróbuj ponownie.'
        },
        en: {
          ...
          closeError: 'An error occurred while closing the catastrophe. Please try again.'
        }
      };
      ...
      const closeCatastrophe = async () => {
        ...
        try {
          const response = await axios.post(`http://localhost:8080/api/catastrophes/close/${catastropheId}`);
